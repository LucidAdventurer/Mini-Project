...
foreach ($answerRows as $r) {
    ...
    elseif (pgBoolGuard($r['is_correct'])) $correct++;
    else $wrong++;
    $d = $r['difficulty'] ?? 'easy';
    if (isset($breakdown[$d])) {
        $breakdown[$d]['total']++;
        if (pgBoolGuard($r['is_correct'])) $breakdown[$d]['correct']++;
    }
}
...
function timeAgoLvl(string $dt): string {
...
function fmtTimeLvl(int $s): string { return floor($s/60).'m '.($s%60).'s'; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .notification-icon:hover{background:rgba(255,255,255,.2)}
        ...
        .notif-dismiss-btn{background:none;border:none;color:var(--text-soft);font-size:13px;line-height:1;padding:2px 5px;border-radius:4px;cursor:pointer;flex-shrink:0;opacity:0;transition:opacity .15s;align-self:flex-start;margin-top:2px}
        ...
        .test-date{font-size:13.5px;color:rgba(255,255,255,.7);margin-bottom:20px;position:relative;z-index:1}
        .levels-row{display:flex;gap:6px;justify-content:center;flex-wrap:wrap;margin-bottom:20px;position:relative;z-index:1}
        .diff-badge{display:inline-flex;align-items:center;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700}
        ...
        .fill-easy{background:linear-gradient(90deg,#10b981,#34d399)}
        .fill-medium{background:linear-gradient(90deg,#f59e0b,#fbbf24)}
        .fill-hard{background:linear-gradient(90deg,#ef4444,#f87171)}
        ...
        .question-header-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;gap:12px}
        .question-meta-row{display:flex;align-items:center;gap:8px;margin-bottom:4px}
        .question-number{font-family:'Sora',sans-serif;font-size:15px;font-weight:700;color:var(--text-mid)}
        .result-badge{padding:5px 12px;border-radius:6px;font-family:'Sora',sans-serif;font-size:11.5px;font-weight:700;display:flex;align-items:center;gap:5px;flex-shrink:0}
        ...
        .btn-report-submit{flex:1;padding:11px;border-radius:10px;border:none;background:linear-gradient(135deg,#0ea5e9,#06b6d4);color:#fff;font-size:13.5px;font-weight:700;cursor:pointer;font-family:'Inter',sans-serif}
...
<nav class="navbar">
    ...
    <div class="nav-profile">
        <div class="notif-dropdown-wrap">
            <button class="notification-icon" onclick="toggleNotifDropdown()">
                <span>🔔</span>
                ...
                <div class="notif-list">
                    ...
                    <div class="notif-item <?= $isUnread ? 'unread' : '' ?>" id="notif-<?= (int)$n['notification_id'] ?>">
                        ...
                        <div class="notif-item-body">
                            ...
                            <div class="notif-item-time"><?= timeAgoLvl($n['created_at']) ?></div>
                        </div>
...
<div class="page-wrapper">
    ...
    <div class="page-content">
        ...
        <div class="results-header">
            <h1 class="test-title-h">🎯 Level Test Result</h1>
            ...
            <div class="quick-stats">
                ...
                <div class="stat-item"><div class="stat-value"><?= $grade ?></div><div class="stat-label">Grade</div></div>
                <div class="stat-item"><div class="stat-value"><?= fmtTimeLvl((int)$attempt['time_taken_sec']) ?></div><div class="stat-label">Time Taken</div></div>
            </div>
        </div>
        ...
        <div class="analysis-grid">
            ...
            <div class="analysis-card">
                <h3 class="analysis-title"><div class="analysis-icon icon-chart">📊</div>Difficulty Breakdown</h3>
                <div class="breakdown-list">
                    <?php
                    $diffColors = ['easy'=>'fill-easy','medium'=>'fill-medium','hard'=>'fill-hard'];
                    foreach ($breakdown as $level => $data):
                        if ($data['total'] === 0) continue;
                        $bpct = round(($data['correct']/$data['total'])*100);
                        $fillCls = $diffColors[$level];
                    ?>
                    <div class="breakdown-item">
                        <div class="breakdown-label"><span class="diff-badge diff-<?= $level ?>"><?= ucfirst($level) ?></span></div>
                        <div class="breakdown-value"><?= $data['correct'] ?>/<?= $data['total'] ?> (<?= $bpct ?>%)</div>
                    </div>
                    <div class="progress-bar"><div class="progress-fill <?= $fillCls ?>" style="width:<?= $bpct ?>%"></div></div>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="analysis-card">
                ...
                <div class="breakdown-list">
                    ...
                    <div class="breakdown-item"><div class="breakdown-label">Total Time Taken</div><div class="breakdown-value"><?= fmtTimeLvl((int)$attempt['time_taken_sec']) ?></div></div>
                    <div class="breakdown-item"><div class="breakdown-label">Avg per Question</div><div class="breakdown-value"><?= fmtTimeLvl($avgSec) ?></div></div>
                    ...
                    <div class="breakdown-item"><div class="breakdown-label">Levels Tested</div><div class="breakdown-value"><?= implode(', ', $levels) ?></div></div>
                </div>
            </div>
        </div>
        ...
        <div class="questions-section">
                    ...
                    <div class="question-header-row">
                        <div>
                            <div class="question-meta-row">
                                ...
                                <span class="diff-badge diff-<?= $row['difficulty'] ?>"><?= ucfirst($row['difficulty']) ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <div class="action-section">
                ...
                <a href="self-assessment.php?tab=level" class="action-btn btn-secondary">🔄 New Level Test</a>
